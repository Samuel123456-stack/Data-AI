...
Essas variáveis são obrigatórias. A aplicação chama `Config.validate()` durante a inicialização e interrompe a execução se alguma chave estiver ausente. `OMDB_API_KEY` é opcional e fica apenas como suporte para a ferramenta legada `agent/tools/omdb.py`.
